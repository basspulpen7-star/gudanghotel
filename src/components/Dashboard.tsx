...
export function Dashboard({ user, profile }: { user: any, profile: any }) {
  ...
  return (
    <div className="space-y-4 md:space-y-8 animate-in fade-in duration-500 p-2 md:p-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 md:gap-4">
        <div>
          ...
          <p className="text-xs md:text-base text-brand-text-muted">Ringkasan operasional Hotel Alia Matraman hari ini</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
        {mainStats.map((stat, i) => (
          <div key={i} className="bg-brand-card p-4 md:p-6 rounded-2xl border border-brand-border hover:border-brand-accent transition-all group">
            <div className="flex justify-between items-start mb-2 md:mb-4">
              <p className="text-[10px] md:text-xs font-bold text-brand-text-muted tracking-wider uppercase">{stat.label}</p>
              <stat.icon className={cn("w-4 h-4 md:w-5 md:h-5 opacity-50 group-hover:opacity-100 transition-opacity", stat.color)} />
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl md:text-4xl font-bold text-white">{stat.value}</span>
              <span className="text-brand-text-muted text-xs md:text-sm">{stat.unit}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {/* Daily Distribution */}
          <div className="bg-brand-card p-4 md:p-6 rounded-2xl border border-brand-border flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center mb-4 md:mb-6">
                <div className="flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 md:w-5 md:h-5 text-purple-500" />
                  <h3 className="text-sm md:text-base font-bold text-white">Barang Keluar Hari Ini</h3>
                </div>
                <span className="text-xl md:text-2xl font-bold text-purple-500">{todayStats.out} <span className="text-xs md:text-sm font-normal text-brand-text-muted">Pcs</span></span>
              </div>
              <p className="text-xs md:text-sm text-brand-text-muted mb-4">Total barang yang didistribusikan ke departemen hari ini.</p>
            </div>
          ...
          <div className="bg-brand-card p-4 md:p-6 rounded-2xl border border-brand-border flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center mb-4 md:mb-6">
                <div className="flex items-center gap-2">
                  <TrendingDown className="w-4 h-4 md:w-5 md:h-5 text-blue-500" />
                  <h3 className="text-sm md:text-base font-bold text-white">Barang Masuk Hari Ini</h3>
                </div>
                <span className="text-xl md:text-2xl font-bold text-blue-500">{todayStats.in} <span className="text-xs md:text-sm font-normal text-brand-text-muted">Pcs</span></span>
              </div>
              <p className="text-xs md:text-sm text-brand-text-muted mb-4">Total stok baru yang diterima dari vendor hari ini.</p>
            </div>
            <div className="pt-4 border-t border-brand-border">
              <div className="flex items-center gap-2 text-xs text-brand-text-muted">
                <Clock className="w-3 h-3" />
          ...
          <div className="md:col-span-2 bg-brand-card p-4 md:p-6 rounded-2xl border border-brand-border border-l-4 border-l-orange-500">
            <div className="flex items-center justify-between mb-4 md:mb-6">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 md:w-5 md:h-5 text-orange-500" />
                <h3 className="text-sm md:text-base font-bold text-white">Peringatan Stok Rendah</h3>
              </div>
